...
\begin{figure}
  ...
  \caption{MQTT 通讯架构图\cite{wukkadada2018comparison}}
  \label{fig:mqtt-arch}
\end{figure}
...
使用这些通信方式进行数据写入有以下几个优点。首先，这些通讯方式的应用比较广泛，市面上存在大量成熟的通讯库可以直接调用，降低了开发者的开发成本。其次，这些通讯方式也对数据的格式没有太大限制，可以方便地传输结构化数据、半结构化数据以及非结构化数据。最后，这些通讯方式也非常灵活，开发者甚至无需使用数据库的 SDK 就可以直接向数据库中写入数据。
